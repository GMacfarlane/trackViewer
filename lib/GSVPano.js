...
GSVPANO.PanoLoader = function (parameters) {
	...
	this.load = function (location, callback) {
		...
		var self = this;
		const radius = 100; // search for panos within this radius (metres)

	    console.log('Fetch panorama within '+radius+'m of lat='+location.lat()+' lng='+location.lng());
		...
		_panoClient.getPanoramaByLocation(location, radius, function (result, status) {
			if (status === google.maps.StreetViewStatus.OK) {
			    ...
			    // todo try again with increased radius

			    const msg = 'Could not fetch panorama: ' + status
				console.log(msg);
